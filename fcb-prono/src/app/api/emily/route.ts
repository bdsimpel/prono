...
export async function POST(request: Request) {
  try {
    ...
    // No deadline check — this is a special late signup
    ...
    // Validate predictions: 30 matches (match 1 already played)
    if (!Array.isArray(predictions) || predictions.length !== 30) {
      return NextResponse.json({ error: 'Alle 30 wedstrijden moeten ingevuld zijn' }, { status: 400 })
    }
