...
export async function POST(request: Request) {
  try {
    ...
    // Validate predictions: 28 matches (3 already played)
    if (!Array.isArray(predictions) || predictions.length !== 28) {
      return NextResponse.json({ error: 'Alle 28 wedstrijden moeten ingevuld zijn' }, { status: 400 })
    }
