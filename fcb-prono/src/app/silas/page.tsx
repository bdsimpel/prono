...
const PLAYED_MATCH_IDS = new Set([1, 2, 3]);
const STORAGE_KEY = "silas-form";
...
export default function MeedoenPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setExistingNames(
      (existingRes.data || []).map((p) => p.display_name.toLowerCase()),
    );
  ...
  const filledExtra = Object.values(extraAnswers).filter(
    (a) => a.trim() !== "",
  ).length;
  const editableMatches = matches.filter(m => !PLAYED_MATCH_IDS.has(m.id));
  ...
  const handleSubmit = async () => {
    ...
    const predArray = editableMatches.map((m) => ({
      matchId: m.id,
    ...
    try {
      const res = await fetch("/api/silas", {
        method: "POST",
  ...
  const grouped = editableMatches.reduce<Record<string, MatchWithTeams[]>>((acc, m) => {
    ...
    return (
      <div className="max-w-lg mx-auto px-6 py-4 md:py-8">
        <div className="text-center mb-6">
          <h1 className="heading-display text-4xl md:text-5xl text-white leading-none">
            DOE <span className="text-cb-blue">MEE</span>
          </h1>
          <p className="mt-2 md:mt-3 text-gray-400 text-xs md:text-sm">
            Voorspel de play-off uitslagen en bewijs dat jij de echte kenner
            bent.
          </p>
        </div>
        ...
        <div className="space-y-2 mb-4 md:hidden">
          <details className="glass-card-subtle group">
            <summary className="p-3 flex items-center justify-between cursor-pointer list-none [&::-webkit-details-marker]:hidden">
            ...
            <div className="px-3 pb-3 space-y-2">
              <div className="flex items-center gap-3">
                <span className="heading-display text-lg text-cb-blue">1</span>
                <span className="text-sm text-gray-300">
                  Voorspel alle wedstrijden
                </span>
              </div>
              <div className="flex items-center gap-3">
                <span className="heading-display text-lg text-cb-blue">2</span>
                <span className="text-sm text-gray-300">
                  Beantwoord de bonusvragen
                </span>
              </div>
              <div className="flex items-center gap-3">
                <span className="heading-display text-lg text-cb-blue">3</span>
                <span className="text-sm text-gray-300">
                  Volg het klassement
                </span>
              </div>
          ...
          <details className="glass-card-subtle group">
            <summary className="p-3 flex items-center justify-between cursor-pointer list-none [&::-webkit-details-marker]:hidden">
              <span className="text-sm text-white font-medium">Regels</span>
              <svg
                className="w-4 h-4 text-gray-500 transition-transform group-open:rotate-180"
                fill="none"
                stroke="currentColor"
        ...
        {/* Desktop: full info sections */}
        <div className="hidden md:block space-y-2 mb-6">
          <div className="glass-card-subtle p-4 space-y-2">
            <div className="flex items-center gap-3">
              <span className="heading-display text-lg text-cb-blue">1</span>
              <span className="text-sm text-gray-300">
                Voorspel alle wedstrijden
            ...
            <div className="flex items-center gap-3">
              <span className="heading-display text-lg text-cb-blue">3</span>
              <span className="text-sm text-gray-300">Volg het klassement</span>
            </div>
          </div>
        ...
        {deadline && (
          <div className="mb-4 md:mb-6 px-4 py-2 md:py-3 rounded-lg text-xs text-gray-400 border border-white/[0.06] text-center">
            Deadline:{" "}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-end justify-between mb-6">
          ...
          <div className="flex items-center gap-1">
            <span className="heading-display text-2xl text-cb-blue font-bold">
              {filledPredictions}
            </span>
                ...
                <div className="space-y-2">
                  {groupMatches.map((match) => {
                    ...
                    return (
                      <div
                        key={match.id}
                        className={`glass-card-subtle p-3 flex items-center gap-1.5 md:gap-2 ${isFilled ? "border-cb-blue/20" : ""}`}
                      >
                        ...
                        <input
                          ...
                          className={`w-9 h-9 md:w-10 md:h-10 text-center bg-cb-dark border rounded-lg text-white font-bold text-sm focus:outline-none focus:border-cb-blue transition-colors shrink-0 ${pred.home !== "" ? "border-cb-blue/40" : "border-white/[0.06]"}`}
                        />
                        ...
                        <input
                          ...
                          onFocus={(e) => e.target.select()}
                          className={`w-9 h-9 md:w-10 md:h-10 text-center bg-cb-dark border rounded-lg text-white font-bold text-sm focus:outline-none focus:border-cb-blue transition-colors shrink-0 ${pred.away !== "" ? "border-cb-blue/40" : "border-white/[0.06]"}`}
                        />
          ...
          {questions.map((q) => {
            ...
            const teamOptions = (() => {
              ...
              if (q.question_key === "meeste_goals_poi") {
                ...
                sorted.sort(
                  (a, b) => (a.standing_rank ?? 99) - (b.standing_rank ?? 99),
                );
              } else if (q.question_key === "beste_ploeg_poi") {
                sorted.sort(
                  (a, b) => (b.points_half ?? 0) - (a.points_half ?? 0),
                );
              }
              return sorted;
            })();
            const getTeamStatLabel = (t: Team) => {
              if (isBeker) return "";
              if (q.question_key === "meeste_goals_poi")
                return `${t.goals_for ?? 0} goals`;
              if (q.question_key === "minste_goals_tegen_poi")
                return `${t.goals_against ?? 0} goals`;
              if (q.question_key === "kampioen")
                return `#${t.standing_rank} · ${t.points_half ?? 0} ptn`;
              if (q.question_key === "beste_ploeg_poi")
                return `${t.points_half ?? 0} ptn`;
              return "";
            };
            const comboboxOptions = teamOptions.map((t) => ({
              name: t.name,
              statLabel: getTeamStatLabel(t),
            }));
            return (
              <div key={q.id} className="glass-card-subtle p-4 md:p-5">
                <label className="block text-sm font-medium mb-3 text-gray-200">
                  {q.question_label}
                  {q.points === 20 && (
                    <span className="ml-2 text-xs text-cb-blue font-bold px-1.5 py-0.5 bg-cb-blue/10 rounded">
                      20 pts
                    </span>
                  )}
                  {q.points === 10 && (
      ...
      return (
        <div className="max-w-lg mx-auto py-8 px-6 space-y-8">
          <div className="glass-card-subtle p-8 md:p-10 text-center">
            <svg
              className="w-16 h-16 text-cb-blue mx-auto mb-4"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
            ...
            <p className="text-gray-500 text-sm mb-8">
              Je voorspellingen zijn opgeslagen en je betaling wordt
    ...
    return (
      <div className="max-w-lg mx-auto py-8 px-6 space-y-6">
        ...
        <div className="text-center mb-2">
          <svg
            className="w-12 h-12 text-cb-blue mx-auto mb-3"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
