...
export default function Navigation() {
  ...
  useEffect(() => {
    supabase.from('settings').select('value').eq('key', 'deadline').single().then(({ data }) => {
      if (data?.value) {
        setIsLocked(Date.now() >= new Date(data.value).getTime())
      }
    })
  }, [supabase])
    ...
    <>
      ...
      <header className="sticky top-0 z-50 bg-[#0a0e14]/95 backdrop-blur-md border-b border-white/[0.06] hidden md:block">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1">
            ...
            >
              {isLocked ? <ChartIcon /> : <PersonAddIcon />}
              {isLocked ? 'Statistieken' : 'Meedoen'}
            </Link>
            {isAdmin && (
              <button
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0a0e14]/95 backdrop-blur-lg border-t border-white/[0.06] z-50">
        <div className="flex justify-around py-2">
          ...
          >
            <span className="[&>svg]:w-5 [&>svg]:h-5">{isLocked ? <ChartIcon /> : <PersonAddIcon />}</span>
            <span>{isLocked ? 'Statistieken' : 'Meedoen'}</span>
          </Link>
          {isAdmin && (
            <Link
              href="/admin"
